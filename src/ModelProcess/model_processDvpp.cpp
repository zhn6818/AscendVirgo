...
*/
#include "model_processDvpp.h"
#include <iostream>
#include "utils.h"
using namespace std;

ModelProcessDvpp::ModelProcessDvpp() : loadFlag_(false), modelId_(0), modelMemPtr_(nullptr), modelMemSize_(0),
                                       modelWeightPtr_(nullptr), modelWeightSize_(0), modelDesc_(nullptr), input_(nullptr), output_(nullptr),
                                       isReleased_(false)
{
}

ModelProcessDvpp::~ModelProcessDvpp()
{
    DestroyResource();
}

void ModelProcessDvpp::DestroyResource()
{
    if (isReleased_)
        return;
    Unload();
...
Result ModelProcessDvpp::LoadModelFromFileWithMem(const char *modelPath)
{
    if (loadFlag_)
    {
...
Result ModelProcessDvpp::CreateDesc()
{
...
void ModelProcessDvpp::DestroyDesc()
{
    if (modelDesc_ != nullptr)
    {
...
Result ModelProcessDvpp::CreateInput(void *input1, size_t input1size,
...
void ModelProcessDvpp::DestroyInput()
{
    if (input_ == nullptr)
    {
        return;
...
Result ModelProcessDvpp::CreateOutput()
{
    if (modelDesc_ == nullptr)
    {
...
void ModelProcessDvpp::DestroyOutput()
{
    if (output_ == nullptr)
    {
        return;
...
Result ModelProcessDvpp::Execute()
{
...
aclmdlDataset *ModelProcessDvpp::GetModelOutputData()
{
    return output_;
}
