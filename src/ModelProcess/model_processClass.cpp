 ...
 */
#include "model_processClass.h"
#include <iostream>
#include "utils.h"
using namespace std;

ModelProcessClassify::ModelProcessClassify() : loadFlag_(false), modelId_(0), modelWorkPtr_(nullptr), modelWorkSize_(0),
                                               modelWeightPtr_(nullptr), modelWeightSize_(0), modelDesc_(nullptr), input_(nullptr), output_(nullptr)
{
}

ModelProcessClassify::~ModelProcessClassify()
{
    UnloadModel();
    DestroyDesc();
    DestroyInput();
...
Result ModelProcessClassify::LoadModel(const char *modelPath)
{
    if (loadFlag_)
    {
...
Result ModelProcessClassify::CreateDesc()
{
...
void ModelProcessClassify::DestroyDesc()
{
    if (modelDesc_ != nullptr)
    {
...
Result ModelProcessClassify::CreateInput(void *inputDataBuffer, size_t bufferSize)
{
...
void ModelProcessClassify::DestroyInput()
{
    if (input_ == nullptr)
    {
        return;
...
Result ModelProcessClassify::CreateOutput()
{
    if (modelDesc_ == nullptr)
    {
...
void ModelProcessClassify::DestroyOutput()
{
    if (output_ == nullptr)
    {
        return;
...
Result ModelProcessClassify::Execute()
{
...
void ModelProcessClassify::UnloadModel()
{
    if (!loadFlag_)
    {
...
aclmdlDataset *ModelProcessClassify::GetModelOutputData()
{
    return output_;
}

Result ModelProcessClassify::GetModelInputWH(int &width, int &height)
{
    if (modelDesc_ == nullptr)
    {
